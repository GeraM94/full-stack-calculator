...
| `main.go` does wiring only; configuration from environment variables | `main.go` holds everything. Flags `-puerto` and `-web`. Refuses to start without `frontend/dist`. Only `ReadHeaderTimeout`; no graceful shutdown | `cmd/server/main.go`: `PORT`, server timeouts, signal-driven graceful shutdown. Remove static file serving | 2 |
...
| Features missing today that the assignment never mentions: `GET /api/v1/operations` with `Operations()`, CORS middleware with `ALLOWED_ORIGIN` | **Deferred** to "What I'd do with more time" (plan, Appendix D). | Nobody consumes the listing endpoint; CORS is never exercised (Vite proxies in development, nginx in Docker). The assignment asks to prioritize correctness and clarity over extra features |
| Graceful shutdown — also never mentioned by the assignment | **Build it** (step 2.7). | Kept by decision on 2026-09-19: about 15 lines, idiomatic for a Go server, and `docker compose down` stops the container with a terminate signal |
| Docker (optional deliverable 4 in the assignment) | **Build it** — Phase 5 stays in full. Backend image `distroless/static`, non-root; frontend image `nginx:alpine`. | Confirmed on 2026-09-19 after checking the assignment. The Go binary is static, so the image needs no shell or package manager; because distroless has no `wget`, the binary performs its own health check through a `healthcheck` subcommand (step 2.7). Rejected: `alpine` for the backend (tools the service does not need); distroless for the frontend (static files need a web server, and there is no official distroless nginx) |
| Container layout | **Two containers** under `docker compose`: nginx (static files and the `/api/` proxy) and the Go API. | Confirmed on 2026-09-19. A container is a process, not a virtual machine: two containers cost the same memory as the same two processes in one. Rejected: one image where Go also serves `dist/` — smaller and a literal reading of "Dockerfile to run frontend + backend together", but it undoes the removal of static file serving above and couples the microservice to the frontend build. Rejected: both processes in one container — needs a supervisor, rules out distroless, saves nothing |
